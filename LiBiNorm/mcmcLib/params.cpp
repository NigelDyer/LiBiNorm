...
paramDescription::paramDescription(std::string name, double min, double max, double initial) :
...
bool paramDescriptionSet::isValid(const dataVec & data) const
{
	if (data.size() != size())
		return false;
...
void paramDescriptionSet::setValues(const dataVec & vals)
{
	for (size_t i = 0; i < min(size(), vals.size()); i++)
			at(i).value = vals[i];
}

dataVec paramDescriptionSet::getvalues() const
{
	dataVec retVal(size());
	for (size_t i = 0;i < size();i++)
		retVal[i] = at(i).value;
